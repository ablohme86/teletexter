...
void lcd_text(const char *text, uint8_t line, uint8_t align) 
{
    ...
    char formatted_text[lcd_width + 1]; // Buffer for formatert tekst
    
    if (align == LEFT) 
    {
        snprintf(formatted_text, lcd_width + 1, "%-.*s", lcd_width, limited_text);
    } 
    else if (align == RIGHT) 
    {
        snprintf(formatted_text, lcd_width + 1, "%*.*s", lcd_width, lcd_width, limited_text);
    } 
    else if (align == CENTER) 
    {
        int padding = (lcd_width - strlen(limited_text)) / 2;
        snprintf(formatted_text, lcd_width + 1, "%*.*s%s%*.*s", padding, padding, "", limited_text, lcd_width - padding - strlen(limited_text), lcd_width - padding - strlen(limited_text), "");
    }
    
    // Fyll resten av linjen med mellomrom hvis nødvendig
    int text_len = strlen(formatted_text);
    for (unsigned int i = text_len; i < lcd_width; i++) 
    {
        formatted_text[i] = ' ';
    }
    formatted_text[lcd_width] = '\0'; // Null-terminer
    
    for (unsigned int i = 0; i < lcd_width; i++) 
    {
        lcd_write(formatted_text[i], 1);
    }
}
